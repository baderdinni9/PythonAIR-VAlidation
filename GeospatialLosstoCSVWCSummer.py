...
for file in files:
    ...
    actualcolumns = ['Employees','DayShiftEmployees','EveningShiftEmployees','NightRepValue','RiskCount','LocationCount']
    columnmissing = set(actualcolumns)-set(data.columns)
    print(columnmissing)
    for column in columnmissing:
            data[column] = 0

    datasum = data[['Employees','DayShiftEmployees','EveningShiftEmployees','NightRepValue','RiskCount','LocationCount']].sum()
